...
func (p *program) Start(s service.Service) error {
	...
	authSuc = Attack{name: "AuthSuc", method: "POST", maxNap: 240, minNap: 1, pause: 4, maxRequests: 40,
		minRequests: 20, url: fmt.Sprintf("http://%s/login?%s", target, "userName=user1&password=User1_123"),
		headers: map[string]string{"Content-Type": "application/x-www-form-urlencoded"}}
	...
	cve20181177 = Attack{name: "cve20181177", method: "GET", maxNap: 10, minNap: 1, pause: 1,
...
func (p *program) run() {
	...
	c.AddFunc("0 */15 * * * *", func() { cve20181177.send() })
